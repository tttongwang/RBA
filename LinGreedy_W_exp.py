...
from LinGreedy_W            import LinGreedy_W
from generate_lambda_LinGD     import W_lambda_list
...
for n in range(W_lambda_list.shape[1]):
    for m in range(W_lambda_list.shape[1]):
        ...
        for r in range(Repeats):
            context_bandits = LinGreedy_W(theta        = theta, 
        ...
        avg_output_ot_b.to_csv("LinGD_bias" + ".csv")
        avg_output_ot_s = pd.DataFrame(std_list)
        avg_output_ot_s.to_csv("LinGD_std" + ".csv")        
        
        
        if (np.mean(std_list_W[0,:])-np.mean(std_list[0,:]) < 0.2) and (np.mean(std_list_W[0,:])-np.mean(std_list[0,:]) >0):
            match_ind += 1
            avg_output_ot_bw = pd.DataFrame(bias_list_W)
            avg_output_ot_bw.to_csv("LinGD_bias_" + 'lambda_' + str(fixed_lambda) +".csv")
            avg_output_ot_sw = pd.DataFrame(std_list_W)
            avg_output_ot_sw.to_csv("LinGD_std_" + 'lambda_' + str(fixed_lambda) +".csv")
...
avg_output_ot_bw = pd.DataFrame(bias_list_W)
avg_output_ot_bw.to_csv("LinGD_bias_W" +".csv")
avg_output_ot_sw = pd.DataFrame(std_list_W)
avg_output_ot_sw.to_csv("LinGD_std_W" +".csv")
