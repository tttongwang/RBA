# -*- coding: utf-8 -*-
"""
Created on Mon Mar  7 23:25:51 2022
...
from LinGreedy              import LinGreedy
from LinGreedy_boot         import LinGreedy_boot
...
def save_contextual_results():
    while True:
        context_bandits = LinGreedy(theta=theta, linucb_alpha=linucb_alpha, feature_d=feature_d, K=K, sample_size=sample_size)
...
def run_boot_exp():
    ...
    for b in range(B):
        ...
        boot_bandits = LinGreedy_boot(theta=theta, linucb_alpha=linucb_alpha,
...
def run_multi_boot_exp():
    ...
    #std_dict = dict.fromkeys((range(K)))
...
avg_output_ot_b.to_csv("LinGD_resample_bias" + ".csv")

avg_output_ot_s = pd.DataFrame(std_summary)
avg_output_ot_s.to_csv("LinGD_resample_bias_std" + ".csv")
