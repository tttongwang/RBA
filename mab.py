...
class Bandits():
	def __init__(self, random_choice, true_prob, sample_size, algo, lin, lin_t, lin_const, gd_eps, sigma, lil_delta, lil_eps, lil_lambda, lil_beta, dp, dp_eps):
	...
	def update(self, t, selected):
		temp_reward = np.random.normal(loc=self.true_prob[selected], scale = self.sigma, size = 1)
	...
	def run(self):
		...
		return self.bandit_stats, self.output_bias, self.output_means, self.output_pulls, self.output_cov, self.output_corr, self.output_reward, self.output_pull_indicator
